"""
Base codes for Agent and its internal state
"""

from enum import Enum
import uuid


class Status(Enum):
    """
    Agent status, following the SIR model
    """
    Susceptible = 's'
    Infected = 'i'
    Recovered_Immune = 'r'
    Death = 'm'


class InfectionSeverity(Enum):
    """
    The Severity of the Infected agents
    """
    No_Contact = "n"
    Exposed = 'e'
    Incubation = 'a'
    Infectious = "i"
    Hospitalization = 'h'
    Severe = 's'
    Immune = "r"


class AgentType(Enum):
    """
    The type of the agent, or the node at the Graph
    """
    Person = 'p'
    Business = 'b'
    House = 'h'
    Government = 'g'
    Healthcare = 'c'
    Supermarked = "s"


class preExistingCondition(Enum):
    '''
    Whether an agent has a preexisting condition
    '''
    Healthy = False
    Ill = True
    # Severe = "s"
    # Physical = "p"
    # Psychologic = "m"


class infectedAtSupermarket(Enum):
    '''
    Where an agent got infected
    '''
    Supermarket = True
    ElseWhere = False


class Agent(object):
    """
    The container of Agent's attributes and status
    """
    def __init__(self, **kwargs):
        self.id = kwargs.get('id', int(uuid.uuid4()))
        self.x = kwargs.get('x', 0)
        """The horizontal position of the agent in the shared environment"""
        self.y = kwargs.get('y', 0)
        """The vertical position of the agent in the shared environment"""
        self.status = kwargs.get('status', Status.Susceptible)
        """The health status of the agent"""
        self.infected_status = kwargs.get("infected_status", InfectionSeverity.No_Contact)
        """The infection severity of the agent"""
        self.infected_time = kwargs.get('infected_time', 0)
        """The time (in days) after the infection"""
        self.age = kwargs.get('age', 0)
        """The age (in years) of the agent"""
        self.social_stratum = kwargs.get('social_stratum', 0)
        """The social stratum (or their quintile in wealth distribution) of the agent"""
        self.wealth = kwargs.get('wealth', 0.0)
        """The current wealth of the agent"""
        self.type = kwargs.get("agent_type", AgentType.Person)
        """The type of the agent"""
        self.environment = kwargs.get('environment', None)
        '''Number of persons infected'''
        self.infected_people = 0
        '''Preexisting conditions'''
        self.health_status = kwargs.get('health_status', preExistingCondition.Healthy)
        '''Days since shopping'''
        self.days_since_shopping = kwargs.get("days_since_shopping", 0)
        '''last x and y position before shopping'''
        self.last_x = kwargs.get('x', 0)
        self.last_y = kwargs.get('y', 0)
        self.shopping_last_iteration = False
        '''infection location'''
        self.infection_location = kwargs.get("infection_location", infectedAtSupermarket.ElseWhere)


    def get_description(self):
        """
        Get a simplified description of the agent health status

        :return: string
        """
        if self.status == Status.Infected:
            return "{}({})".format(self.status.name, self.infected_status.name)
        else:
            return self.status.name

    def __str__(self):
        return str(self.status.name)
