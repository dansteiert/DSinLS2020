...
class Simulation(object):
    def __init__(self, **kwargs):
        ...
        '''incubation period'''
        self.incubation_period = kwargs.get("incubation_period", 6)
        '''effect of preexisting condition'''
        self.preexisting_condition_factor = kwargs.get("preexisting_condition", 1.2)
        '''Recovery time (dependent on chosen interval)'''
    ...
    def pre_existing_condition(self, age):
        age_dependen_conditions = np.random.rand() * (age/100)
        if age_dependen_conditions >= 0.2:
            return preExistingCondition.Ill
        return preExistingCondition.Healthy

    def find_nearest_supermarked(self, agent):
        nearest_dist = np.sqrt(self.height ** 2 + self.length ** 2)
        for i in self.population[-self.supermarked:]:
            dist = distance(agent, i)
    ...
    def create_agent(self, status, agent_type=None):
        """
        ...
        preexisting_condition = self.pre_existing_condition(age)
        days_since_shopping = np.random.randint(1, 5)
        if status == Status.Infected:
            self.population.append(Agent(x=x, y=y, age=age, status=status, social_stratum=social_stratum,
                                         health_status=preexisting_condition,
                                         infected_status=InfectionSeverity.Infectious,
                                         days_since_shopping=days_since_shopping))
        elif agent_type is not None:
            self.population.append(Agent(x=x, y=y, status=status, agent_type=agent_type))
        else:
            self.population.append(Agent(x=x, y=y, age=age, status=status, social_stratum=social_stratum,
                                         health_status=preexisting_condition, days_since_shopping=days_since_shopping))
    ...
    def initialize(self):
        ...
        for i in np.arange(0, self.supermarked):
            self.create_agent(Status.Death, AgentType.Supermarked)
    ...
    def contact(self, agent1, agent2):
        ...
        if agent1.status == Status.Susceptible and agent2.status == Status.Infected and \
                agent2.infected_status in [InfectionSeverity.Infectious, InfectionSeverity.Hospitalization,
    ...
    def move(self, agent, triggers=[]):
        ...
        if agent.status == Status.Death or (agent.status == Status.Infected
                                            and (agent.infected_status in [InfectionSeverity.Hospitalization,
                                                                           InfectionSeverity.Severe])):
            return

        if agent.shopping_last_iteration:
            agent.x = agent.last_x
            agent.y = agent.last_y
            agent.shopping_last_iteration = False
            agent.days_since_shopping += 1
            moved = False

        elif agent.days_since_shopping % 5 == 0:
            x, y = self.find_nearest_supermarked(agent)
            agent.last_x = agent.x
            agent.last_y = agent.y
            agent.days_since_shopping = 0
            agent.shopping_last_iteration = True
            agent.x = x
            agent.y = y
            moved = True
    ...
    def update(self, agent):
        ...
        # changes: health multiplicator added, which increases risk for severity something by 20%

        if agent.infected_status == InfectionSeverity.Immune:
            return
        ...
        if agent.status == Status.Infected:
            ...
            health_multiplicator = 1 if agent.health_status == preExistingCondition.Healthy else self.preexisting_condition_factor
            teste_sub = np.random.random()

            if agent.infected_status == InfectionSeverity.Incubation:
                if agent.infected_time < self.incubation_period:
                    pass
                else:
                    if age_hospitalization_probs[indice] * health_multiplicator > teste_sub:
                        agent.infected_status = InfectionSeverity.Hospitalization
                    else:
                        agent.infected_status = InfectionSeverity.Infectious

            elif agent.infected_status == InfectionSeverity.Infectious:
                if age_hospitalization_probs[indice] * health_multiplicator > teste_sub:
                    agent.infected_status = InfectionSeverity.Hospitalization
            elif agent.infected_status == InfectionSeverity.Hospitalization:
                if age_severe_probs[indice] * health_multiplicator > teste_sub:
                    ...
                    if self.statistics['Severe'] + self.statistics['Hospitalization'] >= self.critical_limit:
                        ...
                        agent.infected_status = InfectionSeverity.Immune

            death_test = np.random.random()
            if age_death_probs[indice] * health_multiplicator > death_test:
                agent.status = Status.Death
                agent.infected_status = InfectionSeverity.Immune
                return

            # if not dead after 20 days, patient recovers
            if agent.infected_time > self.recovery_period:
    ...
    def get_statistics(self, kind='info'):
        ...
        if self.statistics is None:
            ...
            for status in InfectionSeverity:
                self.statistics[status.name] = np.sum([1 for a in self.population if
                                                                a.infected_status == status and
