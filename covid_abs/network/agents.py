...
class Business(Agent):
    ...
    def hire(self, agent):
        if agent.status != Status.Death and agent.infected_status == InfectionSeverity.Incubation:
...
class Person(Agent):
    ...
    def move_to_work(self):
        ...
        if self.infected_status != InfectionSeverity.Incubation:
            return
            ...
            if self.infected_status == InfectionSeverity.Incubation:
                ...
                if age_severe_probs[ix] > test_sub:
                    ...
                    if stats['Severe'] + stats['Hospitalization'] >= self.environment.critical_limit:
                        ...
                        self.infected_status = InfectionSeverity.Incubation
                        if self.house is not None:
