...
def color2(agent):
    ...
    color_dict = {Status.Susceptible: "blue", Status.Recovered_Immune: "green",
                  Status.Death: "black"}
    color_dict_Infected = {InfectionSeverity.Incubation: "yellow", InfectionSeverity.Exposed: "brown",
                           InfectionSeverity.Infectious: "orange", InfectionSeverity.Hospitalization: "lightcoral",
                           InfectionSeverity.Severe: "red", InfectionSeverity.Immune: "darkgreen"}
    if agent.status == Status.Infected:
        # print("infected agent", agent.status.name, color_dict_Infected.get(agent.infected_status, "indigo"),
        #       color_dict_Infected.get(agent.infected_status.name, "indigo"), agent.infected_status.name)
        return(color_dict_Infected.get(agent.infected_status, "indigo"))
    elif agent.type == AgentType.Supermarked:
        return "deeppink"
    return color_dict.get(agent.status, "indigo")
...
def edgecolor(agent):
    if agent.type == AgentType.Supermarked:
        return "k"
    elif agent.health_status == preExistingCondition.Healthy:
        return "g"
    elif agent.health_status == preExistingCondition.Ill:
        return "r"
